...
do
    ...
    trap "kill $!" INT   #if killed kill subordinate commands
...
location="/etc/squadMan/passwd.ini"
...
IFS=""
echo "hi" >/dev/null #random background designed to exit cleanly
...
do
    ...
    mysql -u root -p$password -e "CREATE USER '${users_thing[$num]}'@'localhost' IDENTIFIED BY '${pass[$num]}'"
...
mysql -u root -p$password < /tmp/squadMan/DP_PERMISSIONS.sql
if [ $? != 0 ] #error handling
then 
...
Go to <https://$ip/signIn/> to set create a new member.
Once that is done return here."
declare -i capid
Read -p "Enter the member's CAPID:" capid
capid=${capid//[^0-9]/} #sanitize this So idiots destroy their own thing!
password=""
confirm="hi"
echo "Now create a password for the member.
The password must:
...
-Contain at least 3 of the following:
    ...
    -digits (0-9)
    -special characters (!,?...)"
while [ $password != $confirm ] #while password mismatch
do
    read -s -p "Enter Password:" password
    read -s -p "Confirm Password:" confirm #get password
done 
echo "Select which of the Administrative Staff positions the user is:
[1]: Squadron Commander
[2]: Squadron Deputy Commander
[3]: Information Technology Officer
[4]: Cadet IT Officer"
Read -p "Enter Selection:" staff_pos
case $staff_pos in 
1) staff_pos="CC" ;;
2) staff_pos="CD" ;;
3) staff_pos="ITO" ;;
